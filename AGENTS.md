...
El sistema debe operar offline en una NVIDIA Jetson Nano 4GB. No debe depender de APIs externas ni de internet durante la ejecución en laboratorio.
...
- Modelo Ollama activo en código: `phi4-mini` en `rag/pipeline.py`
...
- Capas frecuentes: Metal1-Metal5, Poly, Ndiff, Pdiff, Contact/CT, Via1-Via4, NWELL, PWELL
- Tipos de regla: spacing/notch, width, enclosure, extension/overlap, área mínima y via/contact
...
- `corpus/annotated/`: ejemplos/reglas anotadas en JSON (`nand2_drc_errors.json`, `ejemplos_anotados_DEF.json`, `xh018_drc_solutions_only_txt_codes.json`).
- `docs/`: documentos de propuesta/diseño.
- `logs/`: logs locales de consultas; no versionar.
- `data/chroma_db/`: base vectorial local; no versionar ni borrar sin confirmación.
...
Comandos equivalentes si el entorno ya está activado:
...
4. Se recuperan chunks semánticos desde ChromaDB.
5. Si la consulta incluye códigos de regla con números, también se hace búsqueda exacta por keyword.
...
- Cuidar memoria: el objetivo es Jetson Nano 4GB, así que evitar modelos grandes o cargas duplicadas.
- No borrar `data/chroma_db/` sin confirmación explícita del usuario.
- No modificar ni truncar `logs/queries.log` salvo que el usuario lo pida.
- Mantener respuestas del asistente en español claro y directo.
- Preferir cambios pequeños y compatibles con la arquitectura actual.
- Si se cambia el modelo Ollama, actualizar también `instructions.md`, `README.md` y este archivo para evitar inconsistencias.
